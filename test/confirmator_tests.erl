...
confirmator_test_() ->
    {foreach,
        ...
        fun stop/1,
        [
             fun generate_ok_autogen/1,
             fun generate_wrong_token_autogen/1,
             fun generate_ok/1,
...
start() ->
    ...
    % TODO add expiry_time!
    application:set_env(confirmator, expiry_time, 3600),
    {ok, AppCtx} = confirmator_backend_mock:start(),
    AppCtx.

stop(_State) ->
    ok.

generate_ok_autogen(AppCtx) ->
  fun() ->
...
generate_wrong_token_autogen(AppCtx) ->
  fun() ->
